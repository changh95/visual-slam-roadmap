...
function createLoadTask(mode, options) {
  return {
    setup: async (testRun, errors, softErrors) => {
      ...
      // Fail when no tests.
      if (options.failOnLoadErrors && !testRun.rootSuite.allTests().length && !testRun.config.cliPassWithNoTests && !testRun.config.config.shard) throw new Error(`No tests found`);
    }
  };
}
