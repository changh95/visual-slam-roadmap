...
- **Handcrafted + learned filters**: Handcrafted derivative-based filters provide anchor structures; learned CNN filters on top of them localize, score, and rank repeatable features. This hybrid keeps the network shallow and cheap.
- **Scale-space inside the network**: A multi-scale pyramid representation is used within the architecture so keypoints are extracted at different levels, giving scale robustness by construction.
- **Repeatability-driven loss**: The loss is designed to detect features that persist across a range of scales and to maximize the repeatability score, rather than to imitate an existing detector.
- **Cheap training data**: Trained on data synthetically generated from ImageNet images (known geometric transformations), so no manual annotation is required.
- **Detector only**: Key.Net detects keypoints; it is typically paired with a learned descriptor such as HardNet to form a complete matching front-end.
